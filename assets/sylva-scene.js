...
(function () {
  ...
  var ticking = false, parOn = false, paused = false;

  /* one rAF for the whole page: it eases the pointer, publishes it, and
     renders the GL scene if there is one. The loop keeps scheduling even
     while paused so a resumed page picks up on the very next frame. */
  function startTick() {
    if (ticking) return;
    ticking = true;
    (function loop() { requestAnimationFrame(loop); if (!paused) tick(); })();
  }
  ...
  function initDock() {
    ...
    root.addEventListener('click', function (e) {
      ...
      if (!item) return;
      e.preventDefault();
      var page = item.getAttribute('data-open');
  ...
  function build() {
    ...
    /* ---- far ridge: same builder, pushed back and washed into the air.
            It dissolves before it reaches the cards (local x 0.5 â†’ 4.0) and
  ...
  /* ── cursor â†’ the plane the root stands in, in each root's own space ── */
  ...
  /* the overlay calls this when a page covers the scene — let the GL rest
     until it is closed again */
  window.__sylvaPause = function (p) { paused = !!p; window.__sylvaPaused = !!p; };
})();